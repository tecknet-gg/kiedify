...
class PhonemeExtractor:
    ...
    def __init__(self, musicDir="/Users/jeevan/Documents/Python/MusicTTS/Music", mfaPath="/Users/jeevan/miniconda3/envs/mfa/bin/mfa", target=10):
        ...
        self.target = target
        self.g2p = G2p()
        #nltk.download('averaged_perceptron_tagger_eng') #download once
        ...
        self.phonemeWeights = { #weights generated by ai, i couldn't really be asked to write them myself - either way weighted interpolation wasn't my brightest idea
...
class PhonemeNode:
    def __init__(self, nodeId, phoneme, start, end, leftContext="", rightContext="", songName=""):
        self.id = nodeId
...
class PhonemeSynth:
    ...
    def calculateTransition(self, nodeA, nodeB):
        if nodeA.songName == nodeB.songName:
            timeGaps = nodeB.start - nodeA.end
    ...
    def calculateCost(self, node, targetPhoneme, prevTargetPhoneme=None, nextTargetPhoneme=None):
        ...
        basePriority = self.weights.get(cleanTarget, 1.0) if self.weights else 1.0
        cost = max(0.05, 1.0 - (basePriority * 0.1))

        if prevTargetPhoneme:
            cleanPrevTarget = "".join([character for character in prevTargetPhoneme if not character.isdigit()])
            if node.leftContext != cleanPrevTarget:
                cost += 1.0

        if nextTargetPhoneme:
            cleanNextTarget = "".join([character for character in nextTargetPhoneme if not character.isdigit()])
            if node.rightContext != cleanNextTarget:
                cost += 1.0

        nodeDuration = node.end - node.start
        if nodeDuration > 0.4:
            cost += (nodeDuration - 0.4) * 2.0
    ...
    def generateStitchMap(self, input):
        ...
        nextPhoneme = targetSequence[0] if len(targetSequence) > 1 else None
        for node in matrix[0]:
            firstStepCost[id(node)] = self.calculateCost(node, targetSequence[0], None, nextPhoneme)
        ...
        for t in range(1, len(targetSequence)):
            ...
            prevPhoneme = targetSequence[t-1]
            nextPhoneme = targetSequence[t+1] if t+1 < len(targetSequence) else None
